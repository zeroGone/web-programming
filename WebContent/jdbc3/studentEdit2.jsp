...
if (request.getMethod().equals("GET")) {
    ...
    String s2 = request.getParameter("radio");
    ...
    if (s1 == null || s1.length() == 0) 
        ...
        response.sendRedirect("studentList1.jsp?pg="+(id+10)/10);
        return;
    }
}
%>
...
<%for(Department d:DepartmentDAO.findAll()){ %>
  	 <%String checked= student.getDepartmentId()==d.getId()?"checked":"";%>
  		<label><input type="radio" name="radio" value="<%=d.getId()%>" <%=checked%>><%=d.getDepartmentName()%></label>
  	<%} %>
  </div>
  <div class="form-group">
    <label>학년</label>
